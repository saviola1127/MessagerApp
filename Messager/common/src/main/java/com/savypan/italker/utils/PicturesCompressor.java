...
public final class PicturesCompressor {
    ...
    public static File loadWithGlideCache(String path) {
        File tmp;
        try {
            tmp = Glide.with(CommonApplication.getInstance())
                    .load(path)
     ...
     *                    需要时，推荐 {{@link net.qiujuer.italker.utils.BitmapUtil#DEFAULT_BUFFER_SIZE}}
     * @param options     批量压缩时复用参数，可调用 {{@link net.qiujuer.italker.utils.BitmapUtil#createOptions()}} 得到
    ...
    public static boolean compressImage(final String srcPath,
        ...
        if (sourceFile.length() <= maxSize && confirmImage(sourceFile, options)) {
            return com.savypan.italker.common.utils.StreamUtil.copy(sourceFile, saveFile);
        }
        ...
        // Doing
        File tempFile = com.savypan.italker.common.utils.BitmapUtil.Compressor.compressImage(realCacheFile, maxSize, minQuality, maxWidth,
                maxHeight, byteStorage, options, exactDecode);

        // Rename to out file
        return tempFile != null && com.savypan.italker.common.utils.StreamUtil.copy(tempFile, saveFile) && tempFile.delete();
    }

    public static boolean confirmImage(File file, BitmapFactory.Options opts) {
        if (opts == null) opts = com.savypan.italker.common.utils.BitmapUtil.createOptions();
